...
var ClubActivityDetails = require('./models/ClubActivity.js');
var ClubMemberDetails = require('./models/ClubMemberDetails.js');
...
ShowStats = require('./res/show_stats.js');
let showStats = new ShowStats(cardsConfig.srcBaseUrl, cardsConfig.contentType);

LeaderBoard = require('./res/leaderboard.js');
let leaderBoard = new LeaderBoard(cardsConfig.srcBaseUrl, cardsConfig.contentType);
...
framework.on('attachmentAction', function (bot, trigger) {
  ...
  // } else if(attachmentAction.inputs.cardType === 'showStats') {
  //   if (attachmentAction.inputs.customRequested) {
...
function renderSelectedCard(bot, cardSelection) {
  ...
  switch (cardSelection) {
    case ('showStats'):
      var clubMemberResult = null;
      var memberList = [];
      ClubMemberDetails.find({}).exec(function(err, docs) { 
        //res.json(docs)
        clubMemberResult = docs;
        console.log("clubMember  " + docs);

        clubMemberResult.map((data, index) => {
          memberList.push({
            "title": data.name,
            "value": data.name
          })
    ...
    case('leaderBoard'):
      ...
      let outputArray = [];
      var hashMap = new Map;
      clubActivityResult.map((data, index) => {
      let count = 1;
      //sample
      for(var employee of clubActivityResult){
          ...
          //up the prev count
          hashMap[name] = hashMap[name] + employee.distance; 
          
          }else{
          hashMap[name] = employee.distance;
          }
      }
        })
        //console.log(docs1);
        ...
        for(var key in hashMap){ 
          ...
          leaderboardString += (count++ + ". " + key + " - " + (hashMap[key]/1000).toFixed(2) + " kms" + "\n");
